...
<div id="filtro_lateral">
    ...
    <div class="categoria" v-for='(item, index) in categorias'>
        ...
        <div :class="{'hide': item.menuAtivo == false}" class="conteudo" v-for='(item_2, index_2) in item.itens'>
            <label>
                <input type="checkbox" @change.prevent='guardaCheck([index, item_2.id])' />&nbsp;&nbsp;<span>@{{ index_2 }}</span>
                <small align='center'>@{{ item_2.qtd }}</small>
            </label>
        </div>
    </div>
</div>
        ...
        guardaCheck:function(item){
            ...
            if (this.arrChecks[tipo] != null) {
                ...
                if (key >= 0) {
                    ...
                    this.arrChecks = [];
                    delete arrAux[tipo][key];
                    for (var i in arrAux) {
                        for (var j in arrAux[i]) {
                            if (this.arrChecks[i] == null) {
                                this.arrChecks[i] = [arrAux[i][j]];
                            }else{
                                this.arrChecks[i].push(arrAux[i][j])
                            }
                        }
                    }
                }else{
                    this.arrChecks[tipo].push(valor)
                }
            }else{
